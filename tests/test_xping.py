...
def test_subprocess_trace_live_parses_hops():
    ...
    seen = []
    # trace_tool()/trace_command() must be mocked too — otherwise this test
    # depends on whether `traceroute`/`tracert` happens to be installed on
    # the machine running it, which is exactly the kind of environment
    # dependency that made this test flaky in CI.
    with patch("xping.diagnostics.trace.trace_tool", return_value="traceroute"):
        with patch("xping.diagnostics.trace.trace_command",
                   return_value=["traceroute", "example.com"]):
...
def test_lookup_socket_fallback():
    ...
    # lookup()'s fallback path calls _raw_query(host, qtype, server) once per
    # record type (1=A, 28=AAAA, 15=MX, 2=NS, 16=TXT) — NOT _socket_resolve,
    # which is unused dead code in the current implementation. Mocking the
