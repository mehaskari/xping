...
from .ansi import (
    BOLD, BRAND_AMBER, BRAND_MINT, BRAND_ROSE, BYELLOW, DIM, c,
)
...
def latency_histogram(rtts: list[float]) -> str:
    ...
    bar_width = 20
    lines = []
    for (label, _lo, _hi, color), count in zip(buckets, counts):
        if count == 0:
            continue
    ...
    empty = width - filled
    bar_color = (
        BRAND_MINT if rtt < 30
        else BRAND_AMBER if rtt < 100
        else BYELLOW if rtt < 300
        else BRAND_ROSE
    )
