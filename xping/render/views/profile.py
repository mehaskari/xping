...
def print_saved(entry) -> None:
    ...
    print()
    print(
        c("  ✔  Use it anywhere a host is expected: ", BRAND_MINT)
...
def print_list(result) -> None:
    if not result.profiles:
        ...
        print(c("  Add one with: ", DIM) + c("xping profile add <name> <target>", BWHITE, BOLD))
        print()
        return

    rows = [
        [p.name, p.target, str(p.port) if p.port else "—", p.note or "—"] for p in result.profiles
    ]
