...
from ..ansi import (
    BOLD, BRAND_AMBER, BRAND_INDIGO, BRAND_MINT,
    BRAND_ROSE, BRAND_SLATE, BRAND_TEAL, BWHITE, DIM, c,
)
...
def print_result(result) -> None:
    ...
    if result.redirects:
        ...
        for hop in result.redirects:
            print(f"  {c(str(hop.status_code), BRAND_AMBER, BOLD)}  {c('→', BRAND_AMBER)}  {c(hop.url, DIM)}")
        print(f"  {c('200', BRAND_MINT, BOLD)}  {c('✔', BRAND_MINT)}  {c(result.final_url or '', BWHITE, BOLD)}")
        print()
    ...
    if result.headers:
        ...
        priority = {"content-type", "server", "cache-control",
                    "content-encoding", "location", "x-powered-by"}
    ...
    # ── Summary line ──────────────────────────────────────────────
    redirect_note = (f"  {c(f'↪  {result.redirect_count} redirect(s)', BRAND_AMBER)}"
                     if result.redirect_count else "")
...
def print_result(result) -> None:
    ...
    # ── Redirect chain ────────────────────────────────────────────
    if result.redirects:
        print(c("  ╔═ Redirect chain ", BRAND_INDIGO, BOLD) + c("─" * 41, DIM))
        for i, hop in enumerate(result.redirects):
            arrow = c("  →", BRAND_AMBER, BOLD)
            print(f"  {c(str(hop.status_code), BRAND_AMBER, BOLD)}{arrow}  {c(hop.url, DIM)}")
        final_arrow = c("  ✔", BRAND_MINT, BOLD)
        print(f"      {final_arrow}  {c(result.final_url or '', BWHITE, BOLD)}")
        print()

    # ── Timing ────────────────────────────────────────────────────
    print(c("  ╔═ Timing ", BRAND_TEAL, BOLD) + c("─" * 49, DIM))
    ttfb = latency_color(result.ttfb_ms) if result.ttfb_ms is not None else c("—", DIM)
    total = latency_color(result.total_ms) if result.total_ms is not None else c("—", DIM)
    print(f"  {'TTFB':<20}  {ttfb}")
    print(f"  {'Total time':<20}  {total}")
    print(f"  {'Body size':<20}  {c(f'{result.body_bytes:,} bytes', BWHITE)}")
    print()

    # ── Headers ───────────────────────────────────────────────────
    if result.headers:
        print(c("  ╔═ Response headers ", BRAND_INDIGO, BOLD) + c("─" * 40, DIM))
        # Sort headers, putting common important ones first
        priority = {"content-type", "server", "cache-control",
                    "content-encoding", "location", "x-powered-by"}
        sorted_headers = sorted(
            result.headers.items(),
        ...
        for key, value in sorted_headers:
            # Truncate very long values (e.g. Set-Cookie) for readability
            display_val = value if len(value) <= 80 else value[:77] + c("…", DIM)
            print(f"  {c(key.lower(), BRAND_TEAL):<40}  {c(display_val, BWHITE)}")
        print()
