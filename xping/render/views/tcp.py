...
def print_summary(result) -> None:
    ...
    success_color = (
        BRAND_MINT if result.success_pct == 100
        else BRAND_AMBER if result.success_pct > 0
        else BRAND_ROSE
    )
    ...
    print_table(
        ...
        [
            ...
            ["Successful", c(str(result.successful),
                             BRAND_MINT if result.successful else BRAND_ROSE)],
    ...
    print()
    print(
        f"  {badge}  {c(result.host, BWHITE, BOLD)}:"
        f"{c(str(result.port), BRAND_INDIGO, BOLD)}"
    )
    print()
