...
def _score_spark(scores: list) -> str:
    ...
    for s in scores:
        idx = int((s - lo) / span * 8)
        grade = (
            "Excellent" if s >= 90 else "Good" if s >= 75
            else "Fair" if s >= 50 else "Poor" if s >= 25
            else "Critical"
        )
...
def print_summary(result) -> None:
    ...
    print(c("  " + "─" * 64, DIM))
    print()
    print(f"  {c(str(result.score), color, BOLD)}{c('/100', DIM)}   "
    ...
    if history and len(history) >= 2:
        ...
        print(f"  {spark}")
        print(c(f"  Last {len(scores)} checks — "
                f"min {min(scores)}  avg {sum(scores)//len(scores)}  max {max(scores)}", DIM))
        print()
