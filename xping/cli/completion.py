...
def _zsh_script() -> str:
    cmds_desc = "\n".join(f"    '{cmd}:xping {cmd}'" for cmd in _COMMANDS)
    flags_cases = ""
    for cmd, flags in _FLAGS.items():
        if not flags:
            continue
...
def _fish_script() -> str:
    lines = [
        "# xping fish completion",
        "# Save to:  ~/.config/fish/completions/xping.fish",
        "# Or run:   xping completion fish > ~/.config/fish/completions/xping.fish",
        "",
    ]
    for cmd in _COMMANDS:
        lines.append(f"complete -c xping -f -n '__fish_use_subcommand' -a {cmd} -d 'xping {cmd}'")
    lines.append("")
    for cmd, flags in _FLAGS.items():
        for flag in flags:
            flag_name = flag.lstrip("-")
            lines.append(f"complete -c xping -n '__fish_seen_subcommand_from {cmd}' -l {flag_name}")
    lines.append("")
    lines.append("# profile subcommands")
    for sub in _PROFILE_SUBCOMMANDS:
        lines.append(
            f"complete -c xping -n '__fish_seen_subcommand_from profile' "
