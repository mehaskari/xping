...
from xping.render import BRAND_AMBER, BRAND_SLATE, BRAND_TEAL, BOLD, DIM, banner, c, error
...
def _print_next_steps(host: str) -> None:
    ...
    print()
    print(c("  ── What else can you do with this host? ", BRAND_SLATE, BOLD)
          + c("─" * 26, DIM))
    print()
    for i, (cmd, desc) in enumerate(_NEXT_STEPS):
        if cmd in ("speedtest",):
...
def _is_bare_host(argv: list[str]) -> str | None:
    ...
    known_commands = {
        ...
        "mtr", "mtu", "profile", "speedtest", "completion", "listen",
        "osdetect", "deps", "about", "-h", "--help", "-V", "--version",
    }
    if len(argv) == 1:
        token = argv[0]
...
def main() -> None:
    ...
    import xping.cli.commands as _cmds
    dispatch = {
        "ping":       _cmds.cmd_ping,
        "trace":      _cmds.cmd_trace,
        "lookup":     _cmds.cmd_lookup,
        "tcp":        _cmds.cmd_tcp,
        "portscan":   _cmds.cmd_portscan,
        "sweep":      _cmds.cmd_sweep,
        "ipscan":     _cmds.cmd_ipscan,
        "all":        _cmds.cmd_all,
        "rdns":       _cmds.cmd_rdns,
        "dnscheck":   _cmds.cmd_dnscheck,
        "tls":        _cmds.cmd_tls,
        "http":       _cmds.cmd_http,
        "whois":      _cmds.cmd_whois,
        "health":     _cmds.cmd_health,
        "mtr":        _cmds.cmd_mtr,
        "mtu":        _cmds.cmd_mtu,
        "profile":    _cmds.cmd_profile,
        "speedtest":  _cmds.cmd_speedtest,
        "listen":     _cmds.cmd_listen,
        "osdetect":   _cmds.cmd_osdetect,
        "completion": _cmds.cmd_completion,
        "deps":       _cmds.cmd_deps,
        "about":      _cmds.cmd_about,
    }
