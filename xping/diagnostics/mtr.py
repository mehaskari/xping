...
def mtr(host: str, max_hops: int = 30, cycles: int = 10,
        timeout: float = 2.0, interval: float = 0.3,
        quiet: bool = False) -> MtrResult:
    ...
    if path is None:
        ...
        if not quiet:
            warn_missing("raw sockets",
                         "using system traceroute/ping (run as root for native mode)")
