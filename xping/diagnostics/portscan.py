...
from xping.render import c, section_header, kv, BRAND_TEAL, BRAND_INDIGO, BRAND_MINT, BOLD
...
def scan_port(ip: str, port: int, timeout: float,
              grab_banners: bool = False) -> PortResult:
    ...
    try:
        ...
        elapsed_ms = (time.perf_counter() - started) * 1000
        return PortResult(port=port, open=False, elapsed_ms=elapsed_ms,
                          service=_service_name(port), error="timeout")
    except OSError as exc:
        elapsed_ms = (time.perf_counter() - started) * 1000
        return PortResult(port=port, open=False, elapsed_ms=elapsed_ms,
                          service=_service_name(port), error=str(exc))


def portscan(host: str, ports: list[int], timeout: float = 0.5,
             workers: int = 100, grab_banners: bool = False,
             quiet: bool = False) -> PortScanResult:
    try:
