...
from xping.render import c, section_header, kv, BRAND_INDIGO
...
def ipscan(target: str, timeout: float = 1.0, workers: int = 128,
           limit: int = 4096, quiet: bool = False) -> IpScanResult:
    try:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_probe_ip, ip, timeout): ip
            for ip in hosts
        }
