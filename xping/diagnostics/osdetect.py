...
from xping.diagnostics.platform_cmds import ping_command, parse_ping_rtt, system_name
from xping.render import BOLD, BRAND_TEAL, BRAND_INDIGO, c, kv, section_header
...
def osdetect(host: str, quiet: bool = False) -> dict:
    ...
    result = {"host": host, "ip": None, "ttl": None,
              "os_guess": None, "reasoning": None, "error": None}
    ...
    if not quiet:
        from xping.render import BRAND_MINT, BRAND_AMBER, BWHITE, DIM
        ...
        print()
        from xping.render.ansi import BRAND_ROSE
        print(c("  ⚠  TTL-based detection is a heuristic — VPNs and proxies "
                "can change the observed TTL.", DIM))
        print()
    return result
