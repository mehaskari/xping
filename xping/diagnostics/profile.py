...
def get(name: str) -> ProfileEntry | None:
    ...
    if not entry:
        return None
    return ProfileEntry(
        name=name, target=entry.get("target", ""), port=entry.get("port"), note=entry.get("note")
    )


def add(
    name: str, target: str, port: int | None = None, note: str | None = None, quiet: bool = False
) -> ProfileEntry | None:
...
def list_profiles(quiet: bool = False) -> ProfileListResult:
    data = _load()
    entries = [
        ProfileEntry(name=name, target=v.get("target", ""), port=v.get("port"), note=v.get("note"))
