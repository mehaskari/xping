...
def _bundle_summary(result: BundleResult) -> None:
    ...
    if dns and dns.ipv4:
        ...
        print(f"  {c('DNS', BRAND_SLATE):<24}  {c('✔', BRAND_MINT)}  "
    ...
    p = result.ping
    if p and p.resolved:
        loss_color = BRAND_MINT if p.loss_pct == 0 else (BRAND_ROSE if p.loss_pct > 20 else BRAND_AMBER)
        ping_ok = p.received > 0
        badge = c("✔", BRAND_MINT) if ping_ok else c("✘", BRAND_ROSE)
        detail = (f"{latency_color(p.avg_rtt)} avg   "
                  f"{c(f'{p.loss_pct:.0f}% loss', loss_color)}")
    ...
    if result.trace:
        ...
        print(f"  {c('Trace', BRAND_SLATE):<24}  {badge}  "
...
def run_bundle(host: str, *, quiet: bool = False) -> BundleResult:
    ...
    result = BundleResult(host=host, lookup=dns, ping=icmp,
                          trace=hops, tcp=tcp_checks)
    if not quiet:
