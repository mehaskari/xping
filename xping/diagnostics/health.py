...
def _score(dns_ms: float, ping_result) -> tuple[int, list[str]]:
    ...
    if avg >= 300:
        score -= 25
        issues.append(f"Average latency is very high ({avg:.0f} ms) — noticeable lag for any interactive use.")
    elif avg >= 100:
        score -= 15
        issues.append(f"Average latency is elevated ({avg:.0f} ms) — fine for browsing, rough for gaming or calls.")
    elif avg >= 30:
        score -= 5
...
def health(host: str, count: int = 8, timeout: float = 2.0,
           quiet: bool = False) -> HealthResult:
