...
import os
import platform
from xping.render import c, BRAND_ROSE, BRAND_AMBER, BRAND_MINT, BRAND_SLATE, DIM, BOLD, BWHITE
from xping.render.errors import error, warn, missing_tool
...
def install_cmd(binary: str) -> str:
    ...
    distro = get_distro()
    return _INSTALL_CMDS.get(binary, {}).get(distro,
           _INSTALL_CMDS.get(binary, {}).get("unknown", f"install {binary}"))
...
def print_deps_status() -> None:
    ...
    from xping.render import print_table, section_header, kv
    ...
    for binary, ok in check_all().items():
        status = c(" ✔ found   ", BRAND_MINT, BOLD) if ok \
                 else c(" ✘ missing ", BRAND_ROSE, BOLD)
