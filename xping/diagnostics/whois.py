...
_FIELD_PATTERNS: dict[str, list[str]] = {
    ...
    "creation_date": [
        ...
        r"domain registration date:\s*(.+)", r"registered on:\s*(.+)",
    ],
    "expiration_date": [
        r"registry expiry date:\s*(.+)", r"expiration date:\s*(.+)",
        r"expiry date:\s*(.+)", r"paid-till:\s*(.+)",
        r"registrar registration expiration date:\s*(.+)",
    ],
    "updated_date": [
        r"updated date:\s*(.+)", r"last modified:\s*(.+)", r"last updated:\s*(.+)",
    ],
}
...
def _find_referral(text: str) -> str | None:
    m = re.search(r"(?:whois server|refer|referralserver):\s*(?:whois://)?(\S+)",
                  text, re.IGNORECASE)
...
def whois(domain: str, quiet: bool = False) -> WhoisResult:
    ...
    try:
        ...
        if not result.raw_text or "no match" in result.raw_text.lower()[:200] \
